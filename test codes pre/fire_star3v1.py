...
model = YOLO("best.pt")  # Kendi .pt dosyan (yolov8n ile eğitilmiş olmalı)
...
# FPS ve kararlılık için sayaçlar
start_time = time.time()
frame_count = 0
fire_lost_count = 0  # Yangını geçici kaybetme filtresi
...
try:
    while True:
        ...
        if len(boxes) > 0:
            fire_lost_count = 0  # Yangın bulundu → sayaç sıfırla
            ...
            if center_x < center_screen - tolerance:
                ...
                sola(50)
                time.sleep(0.1)  # Daha kısa dönsün
                dur()
                ...
                if area > 20000:  # Yangın ortada ve yakınsa dur
