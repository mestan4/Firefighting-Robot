...
# 🔥 Hafif YOLOv8 modeli yükle (yolov8n ile eğitilmiş olmalı)
model = YOLO("fire_model.pt")  # Modelin yolov8n ile eğitilmiş .pt dosyası olmalı
...
try:
    while True:
        ...
        # Her 3 karede 1 tanesini işle (frame skipping)
        if frame_count % 3 != 0:
            continue

        results = model(frame, imgsz=320)[0]  # Daha hızlı çalışsın diye imgsz eklendi
        boxes = results.boxes
