...
try:
    while True:
        ...
        if fire_detected:
            # Ateş var ama kutu da çok yakınsa → sola dön
            if can_detected and mesafe < 20:
                print("→ Ateş var ama önümüzde kutu var, sola 45° dönülüyor")
                motor_sola_don()
                time.sleep(1)
                motor_dur()
                continue  # Bu satır kaldırılmıştı ama tekrar koymak gerekebilir

            if abs(fire_x - frame_center) < 40:
                if mesafe < 30:
                    print("→ Ateş ortalandı ve yeterince yaklaşıldı, DUR")
                    motor_dur()
                    break
                else:
                ...
                print("→ Ateş solda, sağa dönülüyor")
                motor_saga_don()
                time.sleep(0.3)

            else:
                print("→ Ateş sağda, sola dönülüyor")
                motor_sola_don()
                time.sleep(0.3)

        else:
            if can_detected and mesafe < 20:
                print("→ Ateş yok ama kutu var, sola 45° dönülüyor")
                motor_sola_don()
                time.sleep(1)
                motor_dur()
            else:
                print("→ Ateş yok, sağa dönerek ateş aranıyor")
                motor_saga_don()
                time.sleep(0.5)
